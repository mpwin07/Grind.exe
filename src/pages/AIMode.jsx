...
import { Search, ExternalLink, Loader, Brain, TrendingUp, BarChart3, CalendarDays, X, Clock, ArrowRight, Target, Sparkles, ShieldAlert, ShieldCheck, Copy, Check, Code2, } from "lucide-react";
import { AppSidebar } from "@/components/AppSidebar";
import { UsernameDialog } from "@/components/UsernameDialog";
import { fetchProfile, searchProblems, getSolution } from "@/lib/api";
...
function getSessionSearch(username) {
...
function saveSessionTopic(username, topic) {
    if (typeof window === "undefined")
        return;
...
function removeRecentSearch(topic) {
    if (typeof window === "undefined")
        return;
...
export default function AIMode() {
    ...
    const [sessionSearch, setSessionSearch] = useState(null);
    ...
    const solvedSlugs = useMemo(() => profileQ.data?.recent.map((r) => r.titleSlug) ?? [], [profileQ.data]);
    ...
    useEffect(() => {
        if (!username || !submittedTopic || !searchQ.data)
            return;
    ...
    const displayedResults = searchQ.data ?? (sessionSearch?.topic === submittedTopic ? sessionSearch.results : undefined);
    ...
    const handleRemoveRecent = (topic) => {
    ...
    return (<div className="min-h-screen bg-background text-foreground">
      <AppSidebar active={active} onSelect={() => { }} onSettings={() => setDialogOpen(true)} avatarUrl={profileQ.data?.avatar}/>
      <main className="ml-20 p-8 lg:p-12 max-w-[1400px]">
        <Header username={username}/>

        {/* Section 1: Weakness Analysis Panel */}
        {profileQ.data && profileQ.data.topTags.length > 0 && (<section className="mt-10">
        ...
        {profileQ.isLoading && (<section className="mt-10">
        ...
        <div className="mt-8">
          <form onSubmit={handleSearch} className="mb-4">
            <div className="flex gap-3">
              <input type="text" value={searchTopic} onChange={(e) => setSearchTopic(e.target.value)} placeholder="Search by topic... (e.g., hashmap, binary tree, dynamic programming)" className="flex-1 px-6 py-3 bg-card border border-white/10 rounded-lg focus:outline-none focus:border-neon focus:ring-1 focus:ring-neon/50 transition-all font-sans cursor-glow"/>
              <button type="submit" disabled={!searchTopic.trim() || !username || searchQ.isLoading} className="px-8 py-3 bg-primary text-primary-foreground font-display font-bold tracking-wide rounded-lg hover:bg-primary/90 cursor-glow transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                {searchQ.isLoading ? (<>
            ...
            {!username && (<p className="mt-3 text-sm text-yellow-400/80">
                Please{" "}
                <button type="button" onClick={() => setDialogOpen(true)} className="underline hover:text-yellow-300">
          ...
          <RecentSearches searches={recentSearches} onSelect={handleRecentClick} onRemove={handleRemoveRecent}/>

          {searchQ.error && (<div className="p-6 bg-red-500/10 border border-red-500/30 rounded-lg text-red-300 mb-8">
              <p className="font-semibold mb-1">Error:</p>
              <p className="text-sm">{searchQ.error.message}</p>
            </div>)}

          {searchQ.isLoading && (<div className="flex items-center justify-center py-16">
          ...
          {displayedResults && displayedResults.length > 0 && (<ProblemStatsCards results={displayedResults}/>)}

          {displayedResults && displayedResults.length > 0 && (<div className="space-y-4">
              <h3 className="font-display text-lg font-bold mb-6">
                Found {displayedResults.length} problems for "{displayedTopic}"
              </h3>
              {displayedResults.map((problem) => (<ProblemCard key={problem.questionId} problem={problem} topTags={profileQ.data?.topTags}/>))}
            </div>)}

          {displayedResults && displayedResults.length === 0 && !searchQ.isLoading && (<div className="text-center py-12">
              <p className="text-muted-foreground font-sans">No problems found. Try a different topic.</p>
            </div>)}

          {/* Section 2: Difficulty Progression */}
          {displayedResults && displayedResults.length > 0 && (<section className="mt-12">
              <h2 className="font-display text-2xl font-bold mb-6 flex items-center gap-2">
          ...
          {!searchTopic && !displayedResults && (<div className="text-center py-16">
              <p className="text-muted-foreground font-sans text-lg mb-4">
                Enter a topic to find relevant LeetCode problems
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-8">
                {topicSuggestions.map((topic) => {
                const tagData = profileQ.data?.topTags.find((t) => t.tagName.toLowerCase() === topic.toLowerCase());
...
/* ─── Topic Mastery Badge ─── */
function TopicMasteryBadge({ count }) {
    const color = count > 20
        ? "bg-emerald-500/80 text-white"
        : count >= 10
            ? "bg-yellow-500/80 text-white"
            : "bg-white/20 text-white/60";
    return (<span className={`absolute -top-2 -right-2 inline-flex items-center justify-center min-w-[20px] h-5 px-1 text-[10px] font-bold rounded-full ${color}`}>
      {count}
    </span>);
}
/* ─── Section 5: Recent Searches ─── */
function RecentSearches({ searches, onSelect, onRemove, }) {
    ...
    return (<div className="mb-6">
      ...
      <div className="flex flex-wrap gap-2">
        {searches.map((s) => (<span key={s} className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1.5 bg-white/5 border border-white/10 rounded-full text-sm font-sans text-foreground/80 hover:border-neon hover:bg-white/10 cursor-glow transition-all group">
...
function WeaknessAnalysisPanel({ profile }) {
    ...
    const getPoint = (index, radiusFraction) => {
      ...
      <div className="p-8 bg-card border border-white/10 rounded-lg cursor-glow hover:border-neon transition-all flex items-center justify-center">
        <svg viewBox="0 0 300 300" className="w-full max-w-[320px]">
          ...
          {topN.map((_, i) => {
            ...
            return (<line key={`axis-${i}`} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="rgba(255,255,255,0.06)" strokeWidth="1"/>);
        })}

          {/* Data fill */}
          <path d={dataPath} fill="oklch(0.68 0.28 330 / 0.15)" stroke="oklch(0.68 0.28 330)" strokeWidth="2"/>

          {/* Data points */}
          {dataPoints.map((p, i) => (<circle key={`dot-${i}`} cx={p.x} cy={p.y} r="4" fill="oklch(0.68 0.28 330)"/>))}

          {/* Labels */}
          {topN.map((tag, i) => {
            const labelP = getPoint(i, 1.25);
            return (<text key={`label-${i}`} x={labelP.x} y={labelP.y} textAnchor="middle" dominantBaseline="middle" fill="rgba(255,255,255,0.6)" fontSize="10" fontFamily="sans-serif">
                {tag.tagName.length > 12 ? tag.tagName.slice(0, 11) + "…" : tag.tagName}
              </text>);
        })}

          {/* Value labels */}
          {dataPoints.map((p, i) => (<text key={`val-${i}`} x={p.x} y={p.y - 10} textAnchor="middle" fill="oklch(0.68 0.28 330)" fontSize="9" fontWeight="bold" fontFamily="sans-serif">
      ...
      <div className="space-y-6">
        ...
        <div className="p-6 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/20 rounded-lg">
          ...
          <div className="space-y-2">
            {strongAreas.slice(0, 5).map((tag) => (<div key={tag.tagName} className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg">
                <span className="font-sans text-sm">{tag.tagName}</span>
                <div className="flex items-center gap-3">
                  <div className="w-24 h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-400 rounded-full transition-all duration-500" style={{ width: `${Math.min((tag.problemsSolved / maxSolved) * 100, 100)}%` }}/>
                  </div>
        ...
        <div className="p-6 bg-gradient-to-br from-red-500/10 to-orange-500/10 border border-red-500/20 rounded-lg">
          ...
          {weakAreas.length === 0 ? (<p className="text-sm text-muted-foreground font-sans">
              No weak areas detected. Great job!
            </p>) : (<div className="space-y-2">
              {weakAreas.slice(0, 5).map((tag) => (<div key={tag.tagName} className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg">
                  <span className="font-sans text-sm">{tag.tagName}</span>
                  <div className="flex items-center gap-3">
                    <div className="w-24 h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full bg-orange-400 rounded-full transition-all duration-500" style={{ width: `${Math.min((tag.problemsSolved / maxSolved) * 100, 100)}%` }}/>
                    </div>
...
/* ─── Section 3: Problem Stats Cards ─── */
function ProblemStatsCards({ results }) {
    const avgSuccess = Math.round(results.reduce((sum, r) => sum + r.successRate, 0) / results.length);
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      ...
      <div className="p-6 bg-gradient-to-br from-emerald-500/10 to-green-500/10 border border-emerald-500/20 rounded-lg cursor-glow hover:border-neon transition-all">
        ...
        <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden mt-2">
          <div className="h-full bg-gradient-to-r from-emerald-400 to-neon transition-all duration-500" style={{ width: `${avgSuccess}%` }}/>
        </div>
      </div>
      ...
      <div className="p-6 bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-lg cursor-glow hover:border-neon transition-all">
        ...
        <div className="flex h-3 rounded-full overflow-hidden bg-white/10">
          {easyCount > 0 && (<div className="bg-emerald-500 transition-all duration-500" style={{ width: `${(easyCount / total) * 100}%` }}/>)}
          {mediumCount > 0 && (<div className="bg-yellow-500 transition-all duration-500" style={{ width: `${(mediumCount / total) * 100}%` }}/>)}
          {hardCount > 0 && (<div className="bg-red-500 transition-all duration-500" style={{ width: `${(hardCount / total) * 100}%` }}/>)}
        </div>
      </div>
    </div>);
}
/* ─── Section 2: Difficulty Progression ─── */
function DifficultyProgression({ results }) {
    ...
    return (<div className="relative">
      ...
      <div className="space-y-6">
        {steps.map((step, idx) => (<div key={step.label} className="relative flex gap-6">
            {/* Timeline dot */}
            ...
            {/* Card */}
            <div className={`flex-1 p-6 bg-gradient-to-br ${step.bgColor} border ${step.color} rounded-lg cursor-glow transition-all hover:border-neon`}>
              ...
              {step.problems.length === 0 ? (<p className="text-sm text-muted-foreground font-sans italic">
                  No {step.label.split("—")[1]?.trim().toLowerCase()} problems in results
                </p>) : (<div className="space-y-2">
                  {step.problems.map((p) => (<div key={p.questionId} className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg">
                      ...
                      <a href={`https://leetcode.com/problems/${p.titleSlug}/`} target="_blank" rel="noreferrer" className="text-neon hover:text-neon/80 transition-colors">
                        <ExternalLink className="size-3.5"/>
                      </a>
                    </div>))}
                </div>)}

              {idx < steps.length - 1 && (<div className="flex justify-center mt-4 md:hidden">
...
/* ─── Section 4: Study Plan Generator ─── */
function StudyPlanGenerator({ results }) {
    ...
    return (<div className="relative">
      ...
      <div className="space-y-4">
        {days.map(({ day, problem, isRest }) => (<div key={day} className="relative flex gap-6">
            {/* Day number dot */}
            <div className="hidden md:flex flex-col items-center z-10">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ring-4 ring-background ${isRest
            ...
            {/* Card */}
            <div className={`flex-1 p-5 border rounded-lg cursor-glow transition-all ${isRest
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  {isRest ? (<div className="flex items-center gap-2">
                      ...
                      <span className={`inline-block px-2 py-0.5 ${diffColor(problem.difficulty)} text-[10px] font-bold rounded uppercase`}>
                        {problem.difficulty}
                      </span>
                    </div>)}
                </div>
                {!isRest && (<a href={`https://leetcode.com/problems/${problem.titleSlug}/`} target="_blank" rel="noreferrer" className="px-4 py-2 bg-primary text-primary-foreground font-display text-xs font-bold tracking-wide rounded-lg hover:bg-primary/90 transition-all flex items-center gap-1.5 shrink-0 cursor-glow">
...
/* ─── Problem Card ─── */
function ProblemCard({ problem, topTags, }) {
    ...
    return (<div className="p-6 bg-card border border-white/10 rounded-lg hover:border-neon cursor-glow transition-all group">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            const tagData = topTags?.find((t) => t.tagName.toLowerCase() === tag.toLowerCase());
            return (<span key={tag} className="relative inline-block px-2 py-1 bg-white/5 text-xs rounded text-muted-foreground font-sans">
                  {tag}
                  {tagData && (<span className={`ml-1.5 inline-flex items-center justify-center min-w-[16px] h-4 px-1 text-[9px] font-bold rounded-full ${tagData.problemsSolved > 20
        ...
        <div className="flex flex-col gap-2 shrink-0">
          <a href={`https://leetcode.com/problems/${problem.titleSlug}/`} target="_blank" rel="noreferrer" className="px-6 py-3 bg-primary text-primary-foreground font-display text-sm font-bold tracking-wide rounded-lg hover:bg-primary/90 transition-all flex items-center gap-2 cursor-glow">
          ...
          <select value={selectedLanguage} onChange={(e) => handleLanguageChange(e.target.value)} className="px-3 py-2 bg-card border border-white/10 rounded-lg text-xs font-sans focus:outline-none focus:border-neon cursor-glow transition-all">
      ...
      {showSolution && (<div className="mt-6 pt-6 border-t border-white/10">
          {solutionLoading ? (<div className="flex items-center justify-center py-12 text-muted-foreground">
...
function SolutionCodeBlock({ code, language }) {
    ...
    return (<div className="rounded-xl overflow-hidden border border-white/[0.08] bg-[#0c0c15] shadow-lg shadow-black/30">
      {/* Header bar */}
      <div className="flex items-center justify-between px-5 py-3 bg-white/[0.025] border-b border-white/[0.08]">
        ...
        <button onClick={handleCopy} className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-sans font-semibold transition-all duration-200 ${copied
            ? "bg-emerald-500/15 text-emerald-400 ring-1 ring-emerald-500/30"
            : "bg-white/[0.04] text-white/50 hover:bg-white/[0.08] hover:text-white/80 ring-1 ring-white/[0.06]"}`} title="Copy to clipboard">
          {copied ? (<>
