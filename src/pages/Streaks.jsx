...
import { Flame, TrendingUp, Calendar, Target, BarChart3, Lightbulb, Clock, CheckCircle2, Star, Timer, } from "lucide-react";
...
function getSubmissionsForDay(calendar, date) {
    if (!calendar)
        return 0;
...
function sumSubmissions(calendar, startDate, days) {
    if (!calendar)
        return 0;
    let total = 0;
...
export default function Streaks() {
    ...
    useEffect(() => {
        if (profileQ.error)
            toast.error(`Couldn't load LeetCode: ${profileQ.error.message}`);
    }, [profileQ.error]);
    ...
    return (<div className="min-h-screen bg-background text-foreground">
      <AppSidebar active={active} onSelect={() => { }} onSettings={() => setDialogOpen(true)} avatarUrl={profile?.avatar}/>
        ...
        <div className="mt-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section className="lg:col-span-2">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <StatCard label="Best Streak" value={profile?.streak ?? 0} icon={Flame} color="text-red-400"/>
              <StatCard label="Total Solved" value={profile?.totalSolved ?? 0} icon={TrendingUp} color="text-neon"/>
              <AvgTimeCard totalSolved={profile?.totalSolved ?? 0} activeDays={profile?.totalActiveDays ?? 0} loading={profileQ.isLoading}/>
            </div>
          </section>
        </div>
...
/* ─── StreakShowcase ─── */
function StreakShowcase({ streak, loading }) {
    const MAX_STREAK = 365;
    const percentage = Math.min((streak / MAX_STREAK) * 100, 100);
    return (<div className="bg-gradient-to-br from-neon/20 to-primary/20 border border-neon/50 rounded-3xl p-12 text-center cursor-glow">
      ...
      <div className="w-full h-4 bg-white/10 rounded-full overflow-hidden mb-4">
        <div className="h-full bg-gradient-to-r from-neon to-primary shadow-neon transition-all duration-700" style={{ width: `${percentage}%` }}/>
      </div>
      <p className="text-xs text-muted-foreground font-sans">
        {streak} / {MAX_STREAK} day goal
      </p>

      {streak > 0 && (<div className="mt-6 p-4 bg-white/5 border border-white/10 rounded-lg">
...
function StatCard({ label, value, icon: Icon, color, }) {
    return (<div className="p-6 bg-card border border-white/10 rounded-lg cursor-glow hover:border-neon transition-all">
...
/* ─── AvgTimeCard ─── */
function AvgTimeCard({ totalSolved, activeDays, loading }) {
...
function StatisticsDashboard({ profile, loading }) {
    if (loading) {
        return (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[0, 1, 2, 3].map((i) => (<div key={i} className="p-6 bg-white/5 border border-white/10 rounded-lg animate-pulse h-32"/>))}
      </div>);
    }
    if (!profile) {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {stats.map((stat, idx) => (<div key={idx} className={`relative p-8 bg-gradient-to-br ${stat.gradient} border ${stat.borderColor} rounded-xl cursor-glow transition-all hover:border-opacity-60`}>
...
function CalendarHeatmap({ profile, loading }) {
    ...
    return (<div className="p-8 bg-card border border-white/10 rounded-lg overflow-x-auto">
      {/* Month labels */}
      <div className="inline-flex gap-1 min-w-min mb-1 ml-8">
        {(() => {
            ...
            monthPositions.forEach((mp, idx) => {
                ...
                if (left >= lastEnd) {
                    labels.push(<span key={idx} className="text-[10px] text-muted-foreground font-sans absolute" style={{ left: `${left}px` }}>
                  {mp.label}
                </span>);
                    lastEnd = left + 28;
                }
      ...
      <div className="flex gap-0">
        ...
        <div className="flex flex-col gap-1 mr-2 pt-0">
          {["", "Mon", "", "Wed", "", "Fri", ""].map((label, i) => (<div key={i} className="h-3 flex items-center">
        ...
        <div className="inline-flex gap-1 min-w-min">
          {weeks.map((week, weekIdx) => (<div key={weekIdx} className="flex flex-col gap-1">
              {week.map((cell, dayIdx) => (<div key={`${weekIdx}-${dayIdx}`} className={`w-3 h-3 rounded-sm ${getColor(cell.count)} cursor-glow transition-all hover:ring-2 ring-neon/50`} title={`${cell.date.toLocaleDateString()}: ${cell.count} submission${cell.count !== 1 ? "s" : ""}`}/>))}
            </div>))}
        </div>
      </div>
...
function DonutChart({ solved, total, strokeColor, size = 100, }) {
    ...
    return (<svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="transform -rotate-90">
      {/* Background ring */}
      <circle cx={center} cy={center} r={radius} fill="none" stroke="currentColor" strokeWidth="8" className="text-white/10"/>
      {/* Progress ring */}
      <circle cx={center} cy={center} r={radius} fill="none" stroke={strokeColor} strokeWidth="8" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} className="transition-all duration-1000"/>
      {/* Center text */}
      <text x={center} y={center} textAnchor="middle" dominantBaseline="central" className="transform rotate-90 origin-center fill-current text-foreground font-display" fontSize="16" fontWeight="bold">
        {Math.round(percentage)}%
      </text>
    </svg>);
}
function DifficultyBreakdown({ profile, loading }) {
    if (loading) {
        return (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[0, 1, 2].map((i) => (<div key={i} className="p-6 bg-white/5 border border-white/10 rounded-lg animate-pulse h-64"/>))}
      </div>);
    }
    if (!profile) {
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {difficulties.map((diff, idx) => {
            ...
            return (<div key={idx} className={`relative p-8 border rounded-lg cursor-glow transition-all ${diff.bgColor} ${diff.borderColor} hover:border-opacity-60`}>
            ...
            <div className="flex justify-center my-4">
              <DonutChart solved={diff.solved} total={diff.total} strokeColor={diff.strokeColor} size={110}/>
            </div>
...
function MilestonesList({ profile, loading }) {
    if (loading) {
        return (<div className="space-y-6">
        <div className="p-6 bg-white/5 border border-white/10 rounded-lg animate-pulse h-20"/>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[0, 1, 2, 3, 4, 5].map((i) => (<div key={i} className="p-4 bg-white/5 border border-white/10 rounded-lg animate-pulse h-24"/>))}
        </div>
      </div>);
    }
    ...
    const milestones = [
        ...
        },
        {
            ...
            achieved: (profile?.easySolved ?? 0) >= 10 &&
                (profile?.mediumSolved ?? 0) >= 10 &&
                (profile?.hardSolved ?? 0) >= 10,
            progress: Math.min(profile?.easySolved ?? 0, profile?.mediumSolved ?? 0, profile?.hardSolved ?? 0, 10),
            target: 10,
            icon: "🎲",
        },
        {
    ...
    return (<div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 w-full">
            ...
            <div className="w-full h-3 bg-white/10 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-neon to-primary shadow-neon transition-all duration-700 rounded-full" style={{ width: `${(xpInLevel / xpToNextLevel) * 100}%` }}/>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {milestones.map((milestone) => {
            ...
            return (<div key={milestone.title} className={`p-6 border rounded-lg cursor-glow transition-all ${milestone.achieved
              ...
              <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden mb-2">
                <div className={`h-full rounded-full transition-all duration-500 ${milestone.achieved ? "bg-neon" : "bg-white/30"}`} style={{ width: `${Math.min(progressPercent, 100)}%` }}/>
              </div>
...
function DailyCountdown() {
    ...
    return (<div className="col-span-full p-6 bg-gradient-to-br from-neon/20 to-primary/10 border border-neon/40 rounded-xl cursor-glow transition-all hover:border-neon">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <a href="https://leetcode.com/problemset/" target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-neon/20 border border-neon/50 rounded-lg hover:bg-neon/30 transition-all text-neon font-bold text-sm font-sans">
          Go to LeetCode →
        </a>
      </div>
    </div>);
}
function ChallengeSuggestions({ profile, loading }) {
    if (loading) {
        return (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[0, 1, 2, 3].map((i) => (<div key={i} className="p-6 bg-white/5 border border-white/10 rounded-lg animate-pulse h-32"/>))}
      </div>);
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      {/* Challenge cards */}
      {challenges.slice(0, 6).map((challenge, idx) => (<div key={idx} className={`relative p-6 bg-gradient-to-br ${challenge.color} border ${challenge.borderColor} rounded-lg cursor-glow transition-all hover:border-opacity-60 group flex flex-col justify-between`}>
          <div>
            <div className="flex items-start justify-between mb-4">
              <div>
            ...
            {challenge.extra && (<p className="text-xs text-muted-foreground font-sans mb-2">{challenge.extra}</p>)}
          </div>
          ...
          <div className="space-y-3 mt-4">
            <div>
              ...
              <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-neon to-primary transition-all duration-700" style={{ width: `${Math.min(challenge.progress, 100)}%` }}/>
              </div>
            </div>

            <div className="flex items-center justify-between pt-2 border-t border-white/10">
              <span className="text-xs font-bold text-neon">{challenge.reward}</span>
              <a href={challenge.href} target="_blank" rel="noreferrer" className="text-xs px-3 py-1 bg-neon/20 border border-neon/50 rounded hover:bg-neon/30 transition-all text-neon font-bold">
                Start
              </a>
            </div>
          </div>
...
function ActivityTimeline({ profile, loading }) {
    ...
    return (<div className="p-8 bg-card border border-white/10 rounded-lg cursor-glow transition-all hover:border-neon/30">
      <div className="flex items-center justify-between mb-4">
      ...
      <svg width="100%" viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="overflow-visible" preserveAspectRatio="xMidYMid meet">
        {/* Grid lines */}
        {[0, 0.25, 0.5, 0.75, 1].map((ratio) => {
            const y = paddingTop + barAreaHeight * (1 - ratio);
            return (<line key={ratio} x1={paddingLeft} y1={y} x2={chartWidth - paddingRight} y2={y} stroke="currentColor" strokeOpacity={0.07} strokeWidth={1}/>);
        })}
        ...
        {days.map((day, i) => {
            ...
            return (<g key={i} onMouseEnter={() => setHoveredBar(i)} onMouseLeave={() => setHoveredBar(null)} className="cursor-pointer">
              {/* Invisible hit area */}
              <rect x={x} y={paddingTop} width={actualBarWidth} height={barAreaHeight} fill="transparent"/>

              {/* Bar */}
              <rect x={x} y={y} width={actualBarWidth} height={Math.max(barHeight, 2)} rx={3} ry={3} fill={isHovered ? "oklch(0.78 0.25 330)" : "oklch(0.68 0.28 330)"} fillOpacity={isHovered ? 0.9 : day.count > 0 ? 0.7 : 0.15} className="transition-all duration-200"/>

              {/* Glow effect on hover */}
              {isHovered && day.count > 0 && (<rect x={x - 2} y={y - 2} width={actualBarWidth + 4} height={barHeight + 4} rx={4} ry={4} fill="none" stroke="oklch(0.68 0.28 330)" strokeOpacity={0.4} strokeWidth={2}/>)}

              {/* Count above bar */}
              <text x={x + actualBarWidth / 2} y={y - 6} textAnchor="middle" className="fill-current font-display" fontSize="11" fontWeight="bold" fill={isHovered ? "oklch(0.78 0.25 330)" : "oklch(0.68 0.28 330)"} fillOpacity={day.count > 0 || isHovered ? 1 : 0}>
                {day.count}
              </text>

              {/* Day label */}
              <text x={x + actualBarWidth / 2} y={chartHeight - paddingBottom + 16} textAnchor="middle" className="fill-current" fontSize="10" fill="currentColor" fillOpacity={0.5}>
                {day.label}
              </text>

              {/* Date label (shown on hover) */}
              {isHovered && (<text x={x + actualBarWidth / 2} y={chartHeight - paddingBottom + 30} textAnchor="middle" className="fill-current" fontSize="9" fill="oklch(0.68 0.28 330)">
