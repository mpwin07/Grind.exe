...
import { Timer, Clock, TrendingUp, Zap, BarChart3, Calendar, Lightbulb, Trash2, Plus, Trophy, Target, ArrowUp, ArrowDown, Flame, Brain, RefreshCw, } from "lucide-react";
import { AreaChart, Area, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell, } from "recharts";
import { AppSidebar } from "@/components/AppSidebar";
import { UsernameDialog } from "@/components/UsernameDialog";
import { fetchProfile, fetchSubmissions } from "@/lib/api";
const SOLVE_TIMES_KEY = "lc:solve-times";
const STORAGE_KEY = "lc:username";
function getSolveEntries() {
...
function saveSolveEntries(entries) {
    if (typeof window === "undefined")
        return;
    localStorage.setItem(SOLVE_TIMES_KEY, JSON.stringify(entries));
}
function addSolveEntry(entry) {
    const newEntry = {
        ...entry,
...
function formatTimeVerbose(minutes) {
    if (minutes < 60)
        return `${minutes} min`;
    const h = Math.floor(minutes / 60);
...
export default function TimeAnalytics() {
    ...
    const handleSyncLeetCode = useCallback(async (forcedUsername) => {
        ...
        try {
            const submissions = await fetchSubmissions(targetUsername);
            ...
            for (let i = 0; i < sortedSubs.length; i++) {
                ...
                const exists = updatedEntries.some((e) => e.problemTitle.toLowerCase() === sub.title.toLowerCase() && e.date === subDate);
                if (exists)
                    continue;
    ...
    return (<div className="min-h-screen bg-background text-foreground">
      <AppSidebar active="time-analytics" onSelect={() => { }} onSettings={() => setDialogOpen(true)} avatarUrl={profileQ.data?.avatar}/>
      <main className="ml-20 p-8 lg:p-12 max-w-[1400px]">
        <Header username={username}/>

        {/* Quick Entry */}
        <section className="mt-10">
          <QuickEntryForm onAdd={handleAddEntry} syncing={syncing} onSync={() => handleSyncLeetCode()}/>
        </section>

        {/* Summary Stats */}
        {entries.length > 0 && (<section className="mt-10">
        ...
        {entries.length > 0 && (<>
            ...
            {/* Smart Insights */}
            {entries.length >= 5 && (<section className="mt-12">
                <h2 className="font-display text-2xl font-bold mb-6 flex items-center gap-2">
            ...
            <section className="mt-12">
              ...
              <RecentEntriesTable entries={entries} onDelete={handleDeleteEntry}/>
            </section>
          </>)}

        {/* Empty state */}
        {entries.length === 0 && (<div className="mt-16 text-center py-20">
...
/* ─── Quick Entry Form ─── */
function QuickEntryForm({ onAdd, syncing, onSync, }) {
    ...
    return (<form onSubmit={handleSubmit} className="p-6 bg-card border border-white/10 rounded-xl cursor-glow hover:border-neon/30 transition-all">
      <div className="flex items-center justify-between mb-5">
        ...
        <button type="button" onClick={onSync} disabled={syncing} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-neon/10 border border-neon/30 hover:border-neon/60 text-neon text-xs font-bold font-display uppercase tracking-wide hover:bg-neon/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-glow">
          {syncing ? (<>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
        ...
        <div className="md:col-span-4">
          ...
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Two Sum" className="w-full px-4 py-2.5 bg-background border border-white/10 rounded-lg text-sm font-sans focus:outline-none focus:border-neon focus:ring-1 focus:ring-neon/50 transition-all"/>
        </div>
        ...
        <div className="md:col-span-3">
          ...
          <div className="flex gap-2">
            {["Easy", "Medium", "Hard"].map((d) => (<button key={d} type="button" onClick={() => setDifficulty(d)} className={`flex-1 px-3 py-2.5 text-xs font-bold font-display uppercase rounded-lg border transition-all ${diffBtn(d)}`}>
                {d}
              </button>))}
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <input type="number" min="1" max="600" value={timeStr} onChange={(e) => setTimeStr(e.target.value)} placeholder="e.g. 25" className="w-full px-4 py-2.5 bg-background border border-white/10 rounded-lg text-sm font-sans focus:outline-none focus:border-neon focus:ring-1 focus:ring-neon/50 transition-all tabular-nums"/>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-4 py-2.5 bg-background border border-white/10 rounded-lg text-sm font-sans focus:outline-none focus:border-neon focus:ring-1 focus:ring-neon/50 transition-all"/>
        </div>

        {/* Submit */}
        <div className="md:col-span-1">
          <button type="submit" className="w-full px-4 py-2.5 bg-primary text-primary-foreground font-display text-sm font-bold tracking-wide rounded-lg hover:bg-primary/90 transition-all cursor-glow flex items-center justify-center gap-1.5">
      ...
      {timeStr && parseInt(timeStr, 10) > 0 && (<p className="mt-3 text-xs text-muted-foreground font-sans">
          Time: <span className="text-neon font-semibold">{formatTimeVerbose(parseInt(timeStr, 10))}</span>
        </p>)}
    </form>);
}
/* ─── Summary Stats Cards ─── */
function SummaryStatsCards({ entries }) {
    const totalTime = entries.reduce((s, e) => s + e.timeMinutes, 0);
    const fastest = entries.reduce((min, e) => (e.timeMinutes < min.timeMinutes ? e : min), entries[0]);
    const slowest = entries.reduce((max, e) => (e.timeMinutes > max.timeMinutes ? e : max), entries[0]);
    const stats = [
        {
            label: "Total Time",
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((s) => {
            const Icon = s.icon;
            return (<div key={s.label} className={`p-6 bg-gradient-to-br ${s.gradient} border ${s.border} rounded-xl cursor-glow hover:border-neon/40 transition-all`}>
...
function DifficultyAvgChart({ entries }) {
    const data = useMemo(() => {
        const grouped = {
            Easy: [],
            Medium: [],
        ...
        return [
            {
                ...
                avg: grouped.Easy.length
                    ? Math.round(grouped.Easy.reduce((a, b) => a + b, 0) / grouped.Easy.length)
                    : 0,
            ...
            },
            {
                ...
                avg: grouped.Medium.length
                    ? Math.round(grouped.Medium.reduce((a, b) => a + b, 0) /
            ...
            },
            {
                ...
                avg: grouped.Hard.length
                    ? Math.round(grouped.Hard.reduce((a, b) => a + b, 0) / grouped.Hard.length)
                    : 0,
    ...
    return (<div className="p-6 bg-card border border-white/10 rounded-xl">
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} barSize={60}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/>
          <XAxis dataKey="name" tick={{ fill: "rgba(255,255,255,0.5)", fontSize: 12, fontFamily: "Poppins" }} axisLine={{ stroke: "rgba(255,255,255,0.1)" }}/>
          <YAxis tick={{ fill: "rgba(255,255,255,0.5)", fontSize: 12, fontFamily: "Poppins" }} axisLine={{ stroke: "rgba(255,255,255,0.1)" }} label={{
            value: "Minutes",
            angle: -90,
            position: "insideLeft",
...
function DailyTrendChart({ entries }) {
    const data = useMemo(() => {
        const last30 = [];
        for (let i = 29; i >= 0; i--) {
    ...
    return (<div className="p-6 bg-card border border-white/10 rounded-xl">
      <ResponsiveContainer width="100%" height={300}>
        <AreaChart data={data}>
          ...
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/>
          <XAxis dataKey="date" tick={{ fill: "rgba(255,255,255,0.5)", fontSize: 10, fontFamily: "Poppins" }} axisLine={{ stroke: "rgba(255,255,255,0.1)" }} tickFormatter={(v) => {
          ...
          <Area type="monotone" dataKey="total" stroke="oklch(0.68 0.28 330)" strokeWidth={2.5} fill="url(#timeGradient)" dot={false} activeDot={{
            r: 5,
...
/* ─── Weekly / Monthly Trends ─── */
function WeeklyMonthlyTrends({ entries }) {
    ...
    return (<div className="p-6 bg-card border border-white/10 rounded-xl">
      ...
      <div className="flex gap-2 mb-6">
        {["weekly", "monthly"].map((m) => (<button key={m} onClick={() => setMode(m)} className={`px-4 py-2 text-xs font-display font-bold uppercase tracking-wide rounded-lg border transition-all ${mode === m
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)"/>
          <XAxis dataKey="label" tick={{ fill: "rgba(255,255,255,0.5)", fontSize: 11, fontFamily: "Poppins" }} axisLine={{ stroke: "rgba(255,255,255,0.1)" }}/>
          <YAxis tick={{ fill: "rgba(255,255,255,0.5)", fontSize: 12, fontFamily: "Poppins" }} axisLine={{ stroke: "rgba(255,255,255,0.1)" }} label={{
            value: "Minutes",
            angle: -90,
            position: "insideLeft",
          ...
          <Tooltip contentStyle={{
            ...
            fontSize: "13px",
            fontFamily: "Poppins",
        }} formatter={(value) => [`${formatTimeVerbose(value)}`, ""]}/>
...
/* ─── Heatmap Calendar ─── */
function TimeHeatmapCalendar({ entries }) {
    ...
    return (<div className="p-8 bg-card border border-white/10 rounded-xl overflow-x-auto">
      {/* Month labels */}
      ...
      <div className="flex gap-0">
        ...
        <div className="inline-flex gap-1 min-w-min">
          {weeks.map((week, weekIdx) => (<div key={weekIdx} className="flex flex-col gap-1">
              {week.map((cell, dayIdx) => (<div key={`${weekIdx}-${dayIdx}`} className={`w-3 h-3 rounded-sm ${getColor(cell.minutes)} cursor-glow transition-all hover:ring-2 ring-neon/50`} title={`${cell.date.toLocaleDateString()}: ${cell.minutes > 0 ? formatTimeVerbose(cell.minutes) : "No activity"}`}/>))}
            </div>))}
        </div>
      </div>
...
function SmartInsightsPanel({ entries }) {
    const insights = useMemo(() => {
        ...
        for (const diff of ["Easy", "Medium", "Hard"]) {
            const thisWeek = entries.filter((e) => e.difficulty === diff &&
                e.date >= thisWeekStart);
            const lastWeek = entries.filter((e) => e.difficulty === diff &&
                e.date >= lastWeekStart &&
                e.date < lastWeekEnd);
            if (thisWeek.length >= 2 && lastWeek.length >= 2) {
                const thisAvg = thisWeek.reduce((s, e) => s + e.timeMinutes, 0) / thisWeek.length;
                const lastAvg = lastWeek.reduce((s, e) => s + e.timeMinutes, 0) / lastWeek.length;
                if (lastAvg > 0) {
                    const pctChange = Math.round(((lastAvg - thisAvg) / lastAvg) * 100);
                    if (pctChange > 5) {
                        result.push({
        ...
        const bestDay = Object.entries(dayTotals).sort((a, b) => b[1].count - a[1].count)[0];
        if (bestDay) {
            result.push({
        ...
        if (monthEntries.length > 0) {
            const monthTotal = monthEntries.reduce((s, e) => s + e.timeMinutes, 0);
            result.push({
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {insights.map((insight, idx) => (<div key={idx} className="p-5 bg-card border border-white/10 rounded-xl cursor-glow hover:border-neon/30 transition-all flex items-start gap-4">
          <div className={`p-2.5 rounded-lg bg-white/5 ${insight.color} shrink-0`}>
            {insight.icon}
          </div>
...
/* ─── Recent Entries Table ─── */
function RecentEntriesTable({ entries, onDelete, }) {
    ...
    return (<div className="bg-card border border-white/10 rounded-xl overflow-hidden">
      {/* Header */}
      ...
      <div className="max-h-[400px] overflow-y-auto">
        {sorted.slice(0, 50).map((entry) => (<div key={entry.id} className="grid grid-cols-12 gap-4 px-6 py-3.5 border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors items-center">
            ...
            <div className="col-span-2">
              <span className={`inline-block px-2 py-0.5 text-[10px] font-bold uppercase rounded ${diffBadge(entry.difficulty)}`}>
                {entry.difficulty}
              </span>
            </div>
            ...
            <div className="col-span-2 text-right">
              <button onClick={() => onDelete(entry.id)} className="p-1.5 rounded-lg text-muted-foreground hover:text-red-400 hover:bg-red-500/10 transition-all" title="Delete entry">
      ...
      {sorted.length > 50 && (<div className="px-6 py-3 text-xs text-muted-foreground font-sans text-center border-t border-white/[0.06]">
